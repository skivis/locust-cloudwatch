...
# CONFIG VALUES
CW_METRICS_NAMESPACE = "skivis/loadtests/locust"
...
class RequestResult(object):
    ...
    def get_cw_logs_record(self):
        ...
        timestamp = datetime.datetime.utcnow().strftime(
            "%Y/%m/%d %H:%M:%S.%f UTC")  # example: 2016/02/08 16:51:05.123456 CST
        message = ""
        if self.status == STATUS_SUCCESS:
            message = '[timestamp={}] [host={}] [request_type={}] [name={}] [response_time={}] [response_length={}]'.format(
                timestamp, self.host, self.request_type, self.name, self.response_time, self.response_length)
        if self.status == STATUS_FAILURE:
            message = '[timestamp={}] [host={}] [request_type={}] [name={}] [response_time={}] [exception={}]'.format(
                timestamp, self.host, self.request_type, self.name, self.response_time, self.exception)
    ...
    def get_metric_dimensions(self):
        result = [{'Name': 'Request', 'Value': self.name},
...
class CloudWatchConnector(object):
    ...
    def __init__(self, host, namespace, loggroup, logstream, iamrolearn):
        ...
        self.cwlogsclient.create_log_stream(
    ...
    def init_clients(self):
        if self.iamrolearn:
            logging.info(
                "Initializing AWS SDK clients using IAM Role:[{}]".format(self.iamrolearn))
            stsclient = boto3.client('sts')
            stsresponse = stsclient.assume_role(
                RoleArn=self.iamrolearn, RoleSessionName='cwlocustconnector')
            if 'Credentials' in stsresponse:
                accessKeyId = stsresponse['Credentials']['AccessKeyId']
                secretAccessKey = stsresponse['Credentials']['SecretAccessKey']
                sessionToken = stsresponse['Credentials']['SessionToken']
                self.cwlogsclient = boto3.client(
                    'logs',
                    aws_access_key_id=accessKeyId,
                    aws_secret_access_key=secretAccessKey,
                    aws_session_token=sessionToken
                )
                self.cwclient = boto3.client(
                    'cloudwatch',
                    aws_access_key_id=accessKeyId,
                    aws_secret_access_key=secretAccessKey,
                    aws_session_token=sessionToken
    ...
    def on_request_success(self, request_type, name, response_time, response_length, **kwargs):
        request_result = RequestResult(
            self.host, request_type, name, response_time, response_length, "", STATUS_SUCCESS)
        self.response_queue.put(request_result)

    def on_request_failure(self, request_type, name, response_time, exception, **kwargs):
        request_result = RequestResult(
            self.host, request_type, name, response_time, 0, exception, STATUS_FAILURE)
        self.response_queue.put(request_result)
    ...
    def on_quitting(user_count):
        """Event handler that get triggered when when the locust process is
        exiting
        """

    def on_report_to_master(client_id, data):
        """This event is triggered on the slave instances every time a stats
        report is to be sent to the locust master. It will allow us to add our
        extra content-length data to the dict that is being sent, and then we
        clear the local stats in the slave.
        """

    def on_slave_report(client_id, data):
        """This event is triggered on the master instance when a new stats
        report arrives from a slave. Here we just add the content-length to
        the master's aggregated stats dict.
        """

    def loggroup_exists(self):
        result = False
        response = self.cwlogsclient.describe_log_groups(
            logGroupNamePrefix=self.loggroup, limit=1)
        if len(response['logGroups']):
            result = True
        return result
    ...
    def start_cw_loop(self):
        while True:
            ...
            # Refresh AWS clients every 50 minutes, if using an IAM Role
            # (since credentials expire in 1hr)
            ...
            if cw_logs_batch:
                try:
                    ...
                    if 'nextSequenceToken' in response:
                        self.nexttoken = response['nextSequenceToken']
                except Exception as e:
                    logging.error(str(e))

            if cw_metrics_batch:
                try:
                    cwresponse = self.cwclient.put_metric_data(
                        Namespace=self.namespace, MetricData=cw_metrics_batch)
                    logging.debug("PutMetricData response: [{}]".format(
                        json.dumps(cwresponse, indent=4)))
                except Exception as e:
                    logging.error(str(e))

    def get_batch(self):
        """Metric and Log data has to be batched before writing to the
        CloudWatch API.
        """
        result = {}
        cw_logs_batch = []
        ...
        if self.response_queue.qsize() >= self.batch_size:
            for i in range(0, self.batch_size):
                ...
                cw_metrics_batch.append(
                    request_response.get_cw_metrics_status_record())
                cw_metrics_batch.append(
                    request_response.get_cw_metrics_count_record())
                if request_response.get_cw_metrics_response_size_record():
                    cw_metrics_batch.append(
                        request_response.get_cw_metrics_response_size_record())
                if self.usercount:
                    cw_metrics_batch.append(self.usercount.get_metric_data())
...
if __name__ == "__main__":
    ...
    cwconn = CloudWatchConnector(
        host=host,
        namespace=CW_METRICS_NAMESPACE,
        loggroup=CW_LOGS_LOG_GROUP,
        logstream=CW_LOGS_LOG_STREAM,
